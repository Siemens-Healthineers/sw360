...
public class ReleaseRepository extends SummaryAwareRepository<Release> {
    ...
    public List<Release> searchByNameAndVersion(String name, String version, boolean caseInsenstive){
        ...
        if (caseInsenstive) {
            releasesMatchingName = new ArrayList<Release>(getFullDocsById(queryForIdsAsValue("byNameLowerCase", name)));
        } else {
            releasesMatchingName = new ArrayList<Release>(getFullDocsById(queryForIdsAsValue("byname", name)));
        }
