...
public class CycloneDxBOMImporter {
    ...
    private Map<String, List<org.cyclonedx.model.Component>> getVcsToomponentMap(List<org.cyclonedx.model.Component> components) {
        return components.stream().filter(Objects::nonNull)
                .flatMap(comp -> CommonUtils.nullToEmptyList(comp.getExternalReferences()).stream()
                        ...
                        .map(url -> url.replaceAll(SLASH, "."))
                        .map(url -> new AbstractMap.SimpleEntry<>(url, comp)))
                .collect(Collectors.groupingBy(e -> e.getKey(),
                        Collectors.mapping(Map.Entry::getValue, Collectors.toList())));
    }

    public RequestSummary importFromBOM(InputStream inputStream, AttachmentContent attachmentContent, User user) {

        Parser parser;
        if (attachmentContent.getFilename().endsWith(XML_FILE_EXTENSION)) {
        ...
        try {
            ...
            long vcsCount = components.stream().map(org.cyclonedx.model.Component::getExternalReferences)
                    .filter(Objects::nonNull).flatMap(List::stream).map(ExternalReference::getType).filter(typeFilter)
                    .count();
            ...
            if (componentsCount == vcsCount) {
                return importProjectWithComponentReleasesAndPackages(compMetadata, vcsToComponentMap, attachmentContent);

            } else if (componentsCount > vcsCount) {
                requestSummary = importProjectWithComponentReleasesAndPackages(compMetadata, vcsToComponentMap, attachmentContent);
                if (requestSummary.requestStatus.equals(RequestStatus.SUCCESS)) {
                    ...
                    String projectId = messageMap.get("projectId");
                    ...
                    if (CommonUtils.isNotNullEmptyOrWhitespace(packages)) {
                        packageNames.addAll(Arrays.asList(packages.split(JOINER)));
                    }
                    ...
                    Project project = projectDatabaseHandler.getProjectForEdit(projectId, user);
                    Set<String> componentNames = new HashSet<>();
                    Set<String> invalidPakages = new HashSet<>();
                    ...
                    for (org.cyclonedx.model.Component comp : components) {
                        ...
                        if (CommonUtils.isNullOrEmptyCollection(comp.getExternalReferences())) {
                            ...
                            if (CommonUtils.isNotNullEmptyOrWhitespace(pkgAddSummary.getId())) {
                                ...
                                packageNames.add(fullName);
                                continue;
                            }

                        } else {
                            long countVcs = comp.getExternalReferences().stream().map(ExternalReference::getType)
                                    .filter(typeFilter).count();
                            if (countVcs == 0) {
                                Package pkg = createPackage(null, comp, null, licenses);
                                fullName = SW360Utils.getVersionedName(comp.getName(), comp.getVersion());
                                if (pkg == null) {
                    ...
                    messageMap.put("invalidComp", String.join(JOINER, componentNames));
                    messageMap.put("invalidPkgs", String.join(JOINER, invalidPakages));
    ...
    public RequestSummary importProjectWithComponentReleasesAndPackages(org.cyclonedx.model.Component compMetadata,
            Map<String, List<org.cyclonedx.model.Component>> vcsToComponentMap, AttachmentContent attachmentContent)
        ...
        Project project = createProject(compMetadata);

        try {
            AddDocumentRequestSummary projectAddSummary = projectDatabaseHandler.addProject(project, user);
            AddDocumentRequestStatus addStatus = projectAddSummary.getRequestStatus();
            if (AddDocumentRequestStatus.SUCCESS.equals(addStatus) || (AddDocumentRequestStatus.DUPLICATE.equals(addStatus) && CommonUtils.isNotNullEmptyOrWhitespace(projectAddSummary.getId()))) {
                project = projectDatabaseHandler.getProjectForEdit(projectAddSummary.getId(), user);
                log.info("project created successfully: " + project.getId());
            } else {
                summary.setMessage("Duplicate Error: Multiple project with same name and version is already present!");
                return summary;
            }
        } catch (SW360Exception e) {
        ...
        for (Map.Entry<String, List<org.cyclonedx.model.Component>> entry : vcsToComponentMap.entrySet()) {
            ...
            try {
                ...
                for (org.cyclonedx.model.Component bomComp : entry.getValue()) {
                    ...
                    Package pkg = createPackage(entry.getKey(), bomComp, rel, licenses);
                    String pkgName = SW360Utils.getVersionedName(pkg.getName(), pkg.getVersion());

                    try {
                        AddDocumentRequestSummary pkgAddSummary = packageDatabaseHandler.addPackage(pkg, user);
                        // update components specific fields 
                        comp = componentDatabaseHandler.getComponentForEdit(compAddSummary.getId(), user);
                        ...
                        if (CommonUtils.isNotNullEmptyOrWhitespace(pkgAddSummary.getId())) {
                            ...
                            if (AddDocumentRequestStatus.DUPLICATE.equals(pkgAddSummary.getRequestStatus())) {
                                ...
                                if (!rel.getId().equals(dupPkg.getReleaseId())) {
                                    log.error("Release Id of Package from BOM: '%s' and Database: '%s' is not equal!",
                                            rel.getId(), dupPkg.getReleaseId());
                                }
                                countMap.put(PKG_REUSE_COUNT_KEY, countMap.get(PKG_REUSE_COUNT_KEY) + 1);
                                log.info("reusing existing package: " + pkg.getId());
                            } else {
                ...
                RequestStatus updateStatus = componentDatabaseHandler.updateComponent(comp, user);
                if (RequestStatus.SUCCESS.equals(updateStatus)) {
    ...
    private void addPackageWithoutComponent() {
    ...
    private String convertCollectionToJSONString(Map<String, String> map) throws SW360Exception {
        try {
            ...
            throw new SW360Exception(
                    "An exception occured while generating JSON info for BOM import! " + e.getMessage());
        }
    }
    ...
    private Project createProject(org.cyclonedx.model.Component compMetadata) {
        return new Project(CommonUtils.nullToEmptyString(compMetadata.getName()).trim())
                ...
                .setDescription(CommonUtils.nullToEmptyString(compMetadata.getDescription()).trim()).setType("PRODUCT")
    ...
    // private static final String VENDOR_REGEX = "\\.(.+)";
    private Package createPackage(String key, org.cyclonedx.model.Component componentFromBom, Release release, Set<String> licenses) {
        ...
        if (CommonUtils.isNotNullEmptyOrWhitespace(purl)) {
            try {
                ...
                if (PackageManagerType.NPM.toString().equalsIgnoreCase(packageURL.getType())
                    ...
                    packageName = getPackageName(packageURL, componentFromBom, SLASH);
                } else {
                    packageName = getPackageName(packageURL, componentFromBom, ".").replaceAll(SLASH, ".");
                }
