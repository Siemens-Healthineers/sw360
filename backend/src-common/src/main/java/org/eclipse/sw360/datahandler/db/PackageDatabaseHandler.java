...
public class PackageDatabaseHandler extends AttachmentAwareDatabaseHandler {
    ...
    public List<Package> getPackageByIds(Set<String> ids) throws SW360Exception {
        ...
        assertNotEmpty(packages, "Invalid Package Ids, Package not found in db");
        return packages;
    }
    ...
    public List<Package> searchPackages(PackageSearchHandler searchHandler, String searchText) {
        return searchHandler.search(searchText);
    }
    ...
    public RequestStatus updatePackage(Package pkg, User user, boolean forceUpdate) throws SW360Exception {
        removeLeadingTrailingWhitespace(pkg);
        String name = pkg.getName();
        String version = pkg.getVersion();
        if (name == null || name.isEmpty() || version == null || version.isEmpty()) {
        ...
        try {
            ...
            e.printStackTrace();
            log.error(String.format("Invalid PURL for package: %s - Error Message: %s", SW360Utils.printName(pkg)), e);
        ...
        Package actual = packageRepository.get(packageId);
        if (actual == null) {
            throw fail(404, String.format("Could not fetch package from database! id = %s", packageId));
        } else if (changeWouldResultInDuplicate(actual, pkg)) {
            return RequestStatus.DUPLICATE;
        } else if (makePermission(actual, user).isActionAllowed(RequestedAction.WRITE) || forceUpdate) {
        copyImmutableFields(pkg, actual);
        updateModifiedFields(pkg, user.getEmail());

        String actualReleaseId = CommonUtils.nullToEmptyString(actual.getReleaseId());
        String newReleaseId = CommonUtils.nullToEmptyString(pkg.getReleaseId());
        ...
        if (!newReleaseId.equals(actualReleaseId)) {
            ...
            try {
                if (CommonUtils.isNotNullEmptyOrWhitespace(actualReleaseId)) {
                    ...
                    Response resp = packageRepository.updatewithResponse(pkg);
                    // Update the underlying release, if it contain package id
                    if (resp.getStatusCode() == HttpStatus.SC_CREATED) {
                        ...
                        log.error(String.format("Failed to update package: %s with id: %s", SW360Utils.printName(pkg), packageId));
                ...
                if (CommonUtils.isNotNullEmptyOrWhitespace(newReleaseId)) {
                    ...
                    // once we are sure that release exists, update the package
                    Response resp = packageRepository.updatewithResponse(pkg);
                    // Update the underlying release, if it does not contain package id
                    if (resp.getStatusCode() == HttpStatus.SC_CREATED) {
                        if (!packageIds.contains(packageId)) {
                ...
                e.printStackTrace();
                log.error(String.format("Invalid release id %s while adding package %s ", newReleaseId, SW360Utils.printName(pkg)), e);
                return RequestStatus.INVALID_INPUT;
            }
        } else {
        ...
        databaseHandlerUtil.addChangeLogs(pkg, actual, user.getEmail(), Operation.UPDATE, attachmentConnector, Lists.newArrayList(), null, null);
    ...
    public RequestStatus deletePackage(String id, User user, boolean forceDelete) throws SW360Exception {
        ...
        // Remove the package if the user is allowed to do it by himself
        if (makePermission(pkg, user).isActionAllowed(RequestedAction.DELETE) || forceDelete) {
            RequestStatus status = removePackageAndCleanUp(pkg, user);
    ...
    private RequestStatus removePackageAndCleanUp(Package pkg, User user) throws SW360Exception {
        ...
        if (CommonUtils.isNotNullEmptyOrWhitespace(releaseId)) {
            ...
            if (CommonUtils.isNotEmpty(release.getPackageIds()) && release.getPackageIds().contains(pkg.getId())) {
                release.getPackageIds().remove(pkg.getId());
    ...
    public RequestSummary importCycloneDxFromAttachmentContent(User user, String attachmentContentId)
        ...
        try {
            ...
            try (final InputStream inputStream = attachmentStreamConnector
                ...
                return cycloneDxBOMImporter.importFromBOM(inputStream, attachmentContent, user);
            }
        } catch (IOException e) {
