...
public class PackageHandler implements PackageService.Iface {
    ...
    PackageHandler() throws IOException {
        handler = new PackageDatabaseHandler(DatabaseSettings.getConfiguredClient(), DatabaseSettings.COUCH_DB_DATABASE, DatabaseSettings.COUCH_DB_ATTACHMENTS);
    ...
    @Override
    public RequestSummary importCycloneDxFromAttachmentContent(User user, String attachmentContentId, String projectId) throws TException {
    ...
    @Override
    public RequestStatus updatePackageWithForceFlag(Package pkg, User user, boolean forceUpdate) throws TException {
        assertNotNull(pkg);
        assertId(pkg.getId());
        assertUser(user);

        return handler.updatePackage(pkg, user, forceUpdate);
    }
    ...
    @Override
    public RequestStatus deletePackageWithForceFlag(String id, User user, boolean forceDelete) throws TException {
        assertId(id);
        assertUser(user);

        return handler.deletePackage(id, user, forceDelete);
    }
