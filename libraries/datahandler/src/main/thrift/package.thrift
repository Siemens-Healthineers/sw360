...
service PackageService {
     ...
     */
    list<Package> getPackageByIds(1: set<string> packageIds);
     ...
     */
    set<Package> getPackagesByReleaseIds(1: set<string> releaseIds);

    /**
     * Get list of all the Package by release id
     */
    set<Package> getPackagesByReleaseId(1: string releaseId);
     ...
     * Update Package
     */
    RequestStatus updatePackage(1: Package pkg, 2: User user);

    /**
     * try to update a package as a user, if user has no permission, a moderation request is created
     ...
     */
    RequestStatus deletePackage(1: string pacakgeId, 2: User user);

    /**
     * try to delete a package as a user, if user has no permission, a moderation request is created
     ...
     */
    RequestSummary importCycloneDxFromAttachmentContent(1: User user, 2: string attachmentContentId);
}
