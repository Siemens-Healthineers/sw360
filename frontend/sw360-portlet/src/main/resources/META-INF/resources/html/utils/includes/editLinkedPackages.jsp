...
<button type="button" class="btn btn-secondary" id="addLinkedPackages"><liferay-ui:message key="click.to.add.packages" /></button>
<table class="table edit-table mt-1" id="LinkedPackagesInfo">
    ...
    <thead>
        <tr>
            ...
            <th><liferay-ui:message key="package.manager.type" /></th>
            <th></th>
        </tr>
    </thead>
...
require(['jquery', 'modules/dialog'], function($, dialog) {
    $('#LinkedPackagesInfo').on('click', "svg[data-action='delete-package']", function(event) {
        ...
        $dialog = dialog.confirm('danger', 'question-circle', '<liferay-ui:message key="delete.link.to.release" />', '<p><liferay-ui:message key="do.you.really.want.to.remove.the.link.to.package.x" /></p>', '<liferay-ui:message key="delete.link" />', {
