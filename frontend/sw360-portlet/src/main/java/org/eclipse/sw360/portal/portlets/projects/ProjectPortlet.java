...
public class ProjectPortlet extends FossologyAwarePortlet {
    ...
    private void importBom(ResourceRequest request, ResourceResponse response) {
        ...
        String bomtype = request.getParameter(BOM_TYPE);
        String projectId = request.getParameter(PROJECT_ID);
        String docId = request.getRenderParameters().getValue(PROJECT_ID);
        ...
        try {
            ...
            if (isSPDX) {
                ...
                requestSummary = packageClient.importCycloneDxFromAttachmentContent(user, attachmentContentId, projectId);
            }

            boolean isFailure = RequestStatus.FAILURE.equals(requestSummary.getRequestStatus());
            ...
            if (isSPDX) {
                ...
                try {
                    ...
                    log.error("JSON Exception occured, maybe the 'RequestSummary' message is not in JSON form: "
                            + e.getMessage());
                }
            }
            if (isFailure) {
        ...
        if (id != null) {
            try {
                ...
                if (project.getPackageIdsSize() > 0) {
                    addPackagesToRequest(request, project.getPackageIds());
                }
                request.setAttribute(CRITICAL_CR_COUNT, criticalCount);
    ...
    private void loadLinkedPackages(ResourceRequest request, ResourceResponse response) throws IOException, PortletException {
        ...
        String projectId = parameters.getValue(PortalConstants.DOCUMENT_ID);
        if (CommonUtils.isNotNullEmptyOrWhitespace(projectId)) {
            ...
            try {
                ...
                if (CommonUtils.isNotEmpty(project.getPackageIds())) {
                    ...
                    JSONArray packagesData = getPackageData(packages);
                    final JSONObject jsonResult = createJSONObject();
