...
public class PackagePortlet extends Sw360Portlet {
    ...
    private static final int PACKAGE_DT_ROW_PACKAGE_MANAGER_TYPE = 4;
    ...
    private void servePackageList(ResourceRequest request, ResourceResponse response) throws IOException, PortletException {
        ...
        try {
            ...
            log.error("An error ocured while writing Package list JSON: ", e);
        }
    }
    ...
    private Map<String, Set<String>> getPackageFilterMap(PortletRequest request) {
        ...
        for (Package._Fields filteredField : packageFilteredFields) {
            String parameter = parameters.getValue(filteredField.toString());
            if (!isNullOrEmpty(parameter) && !(filteredField.equals(Package._Fields.PACKAGE_MANAGER_TYPE)
        ...
        if (CommonUtils.isNotNullEmptyOrWhitespace(orphanSearch)) {
            filterMap.put("orphan", Collections.emptySet());
        }
        return filterMap;
    }
    ...
    public JSONArray getPackageData(ResourceRequest request, List<Package> packageList, PaginationParameters packageParameters, Map<String, Set<String>> filterMap) {
        ...
        for (int i = start; i < count; i++) {
            ...
            jsonObject.put("relName", "");
            jsonObject.put("relCS", "");
            if (CommonUtils.isNotNullEmptyOrWhitespace(relId)) {
                try {
                    ...
                    log.error(String.format("An error occure while getting release <id> %s info of a package <id> %s", relId, pkg.getId()), e);
                } catch (TException e) {
                    log.error(String.format("An error occure while getting release <id> %s info of a package <id> %s", relId, pkg.getId()), e);
                }
            }
            ...
            jsonObject.put("pkgMgrType", ThriftEnumUtils.enumToString(pkg.getPackageManagerType()));
    ...
    private List<Package> sortPackageList(List<Package> packageList, PaginationParameters packageParameters) {
        ...
        switch (packageParameters.getSortingColumn().orElse(PACKAGE_DT_ROW_NAME)) {
            ...
            case PACKAGE_DT_ROW_LICENSES:
                ...
                break;
            case PACKAGE_DT_ROW_PACKAGE_MANAGER_TYPE:
                Collections.sort(packageList, compareByPackageManagerType(isAsc));
                break;
            default:
                break;
        }
    ...
    private Comparator<Package> compareByPackageManagerType(boolean isAscending) {
        Comparator<Package> pkgManagerComparator = Comparator.comparing(p -> p.getPackageManagerType().toString());
        return isAscending ? pkgManagerComparator : pkgManagerComparator.reversed();
    }
    ...
    public void updatePackage(ActionRequest request, ActionResponse response) throws PortletException, IOException {
        String packageId = request.getRenderParameters().getValue(PACKAGE_ID);
            ...
            if (packageId != null) {
                ...
                if (RequestStatus.DUPLICATE.equals(requestStatus) || RequestStatus.NAMINGERROR.equals(requestStatus)) {
                    if (RequestStatus.DUPLICATE.equals(requestStatus))
                        ...
                        setSW360SessionError(request, ErrorMessages.INVALID_PURL_OR_LINKED_DOCUMENT);
